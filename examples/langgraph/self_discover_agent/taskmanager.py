...
from agent import LangGraphA2AWrapperAgent, TaskInput
...
class AgentTaskManager(InMemoryTaskManager):
    def __init__(self, agent: LangGraphA2AWrapperAgent, notification_sender_auth: PushNotificationSenderAuth):
        super().__init__()
        self.agent = agent
    ...
    def _validate_request(self, request: Union[SendTaskRequest, SendTaskStreamingRequest]) -> JSONRPCResponse | None:
        ...
        if not utils.are_modalities_compatible(
            task_send_params.acceptedOutputModes, LangGraphA2AWrapperAgent.SUPPORTED_CONTENT_TYPES
        ):
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        task_input = self._build_task_input(request.params)
        try:
